...
const fund = (module.exports.fund = web3 => {
  // Fund ETH accounts
  ...
  if (ethAccounts.length) {
    logMessage('Sending 50 ETH to the following addresses...', 'info');
    ethAccounts.forEach((addr, i) => {
      try {
        ...
        logMessage(`    ${addr} <- from ${web3.eth.accounts[i]}`);
      } catch (error) {
        logMessage(`    Error sending funds to ${addr} : ${error}`, 'error');
      }
    });
  } else {
